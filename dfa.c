...
static void
DFA_ensureCapacity(struct DFA *dfa, unsigned int newMaxNumSymbols, state_t newMaxNumStates) {
...
{
  DFA_ensureCapacity(dfa, label, end);
...
{
  DFA_ensureCapacity(dfa, label, MAX(start, end));
  /* FIXME where does this go? */
 ...
 * Treat transitions to "error" as don't-cares.
 */
...
DFA_toKISS2(struct DFA *dfa, FILE *file)
{
  ...
  /* Count the number of transitions, ignoring those to the error state.
   * Also count the number of symbols. */
  unsigned int p = 0;
  unsigned int sym = 0;
  for(unsigned int s = 0; s < dfa->numStates; s++) {
    for(unsigned int x = 0; x < dfa->numSymbols; x++) {
  ...
  /* Add on the number of initial transitions. */
  for(unsigned int x = 0; x < dfa->numSymbols; x++) {
    if(dfa->initial[x] >= 0) {
      p++;
      if(x > sym) sym = x;
  ...
  fprintf(file, ".i %d\n", sym + 1); // number of inputs
  fprintf(file, ".o 1\n"); /* one output */
  fprintf(file, ".p %d\n", p);
  fprintf(file, ".s %d\n", dfa->numStates + 1); /* number of states + initial state */
  fprintf(file, ".r " INIT_STATE "\n"); /* reset (initial) state */

  /* Initial transitions. */
  fprintf(file, "# Initial transitions\n");
  for(unsigned int x = 0; x <= sym; x++) {
    int end;

    if((end = dfa->initial[x]) >= 0) {
      for(unsigned int i = 0; i <= sym; i++) {
        fprintf(file, i == x ? "1" : "0");
      }
      fprintf(file, " " INIT_STATE " st%d %c\n",
              end, BitSet_isSet(dfa->satBit, end) ? '1' : '0');
    }
  }

  /* Later transitions. Each line includes all symbols. One-hot encoding. */
    ...
    for(unsigned int x = 0; x <= sym; x++) {
      ...
      if((end = TRANS(dfa, s, x)) >= 0) {
        ...
        fprintf(file, " st%d st%d %c\n",
                s, end, BitSet_isSet(dfa->satBit, end) ? '1' : '0');
      }
    }
  }
  fprintf(file, ".e\n");
